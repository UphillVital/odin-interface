const ODIN_STORAGE_KEY = "odin_lessons_v3_14";
...
function parseRows(raw, expectedParts){return String(raw||"").split(/\\n+/).map(line=>line.trim()).filter(Boolean).map(line=>line.split("|").map(part=>part.trim())).filter(parts=>parts.length>=expectedParts)}
function collectInput(){return{title:getValue("title"),goal:getValue("goal"),examples:parseRows(getValue("examples"),3),vocab:parseRows(getValue("vocab"),2)}}
function runModeGuard(actionName){log("MODE_GUARD_STARTED");["SON","QA","NN","PLAN","BUILD","TEST","FIX","GIT"].forEach(mode=>log("MODE_OK: "+mode));log("MODE_GUARD_PASSED");return true}
...
function buildLesson(model){
  ...
  return [
    ...
    'table{width:100%;border-collapse:collapse}td,th{border-bottom:1px solid #e2e8f0;padding:8px;text-align:left}.badge{display:inline-block;background:#dcfce7;color:#166534;border-radius:999px;padding:5px 9px;font-weight:800}',
    '</style></head><body class="hl-topic-mode">',
    '<h1>'+esc(model.title)+'</h1><span class="badge">ODIN v3.14 REVIEW READY</span>',
    ...
    '<section id="practice"><h2>5. Практика</h2><p>Натискай на приклади, приховуй переклади і відтвори речення самостійно.</p></section>',
    '<section id="qa-marker"><h2>QA marker</h2><p>Урок пройшов ODIN v3.14 REVIEW PIPELINE.</p></section>',
...
function hardQaCheck(model,html){
  ...
  model.vocab.forEach((w,i)=>{const n=i+1;if(!w[0]||w[0].length<2)error("Словник "+n+": DE відсутнє.");if(!w[1]||w[1].length<2)error("Словник "+n+": UA відсутній.")});
  ...
  info("REVIEW layer ready: NEW / REVIEW / LEARNED.");
...
function renderDownload(html){const blob=new Blob([html],{type:"text/html;charset=utf-8"});const url=URL.createObjectURL(blob);document.getElementById("download").innerHTML='<a class="download" href="'+url+'" download="odin_lesson_v3_14.html">Завантажити урок</a>'}
...
function modeStudy(){ODIN_STATE.showDP=false;ODIN_STATE.showSD=true;ODIN_STATE.uxMode="STUDY";applyMode();applyHighlightMode("TOPIC");log("MODE_STUDY")}
function modeTest(){ODIN_STATE.showDP=false;ODIN_STATE.showSD=false;ODIN_STATE.uxMode="TEST";applyMode();applyHighlightMode("OFF");log("MODE_TEST_ONLY_DE")}
function modeFull(){ODIN_STATE.showDP=true;ODIN_STATE.showSD=true;ODIN_STATE.uxMode="FULL";applyMode();applyHighlightMode("ALL");log("MODE_FULL")}
...
function saveCurrentLesson(){
  ...
  const record={id:"lesson_"+Date.now(),title:ODIN_STATE.lastModel.title,createdAt:new Date().toLocaleString(),html:ODIN_STATE.lastHtml,model:ODIN_STATE.lastModel,qaStatus:ODIN_STATE.lastQa.hasWarnings?"QA_PASSED_WITH_WARNINGS":"QA_PASSED",reviewStatus:"NEW",reviewCount:0,lastReviewed:null};
  lessons.unshift(record); setLessons(lessons); dataLog("LESSON_SAVED: "+record.title); renderLessonsList();
}
...
function setReviewStatus(index,status){
  const lessons=getLessons(); if(!lessons[index])return;
  lessons[index].reviewStatus=status; lessons[index].lastReviewed=new Date().toLocaleString();
  lessons[index].reviewCount=(lessons[index].reviewCount||0)+1;
  setLessons(lessons); dataLog("REVIEW_STATUS_SET: "+lessons[index].title+" → "+status); renderLessonsList();
}
function renderLessonsList(){
  const lessons=getLessons();
  const filtered=REVIEW_FILTER==="ALL"?lessons:lessons.filter(l=>(l.reviewStatus||"NEW")===REVIEW_FILTER);
  const box=document.getElementById("lessonsList");
  if(filtered.length===0){box.innerHTML='<p class="muted">Немає уроків для цього фільтра.</p>';dataLog("LESSONS_LIST_EMPTY: "+REVIEW_FILTER);return}
  box.innerHTML=filtered.map((lesson)=>{
    ...
    return [
      ...
      '<h3>'+esc(lesson.title)+' <span class="status-pill '+statusClass(status)+'">'+status+'</span></h3>',
      '<div class="lesson-meta">'+esc(lesson.createdAt)+' · '+esc(lesson.qaStatus)+' · reviews: '+(lesson.reviewCount||0)+'</div>',
      '<button type="button" onclick="openLesson('+realIndex+')">Відкрити</button>',
      '<button type="button" onclick="exportSavedLesson('+realIndex+')">Export</button>',
      '<button type="button" onclick="setReviewStatus('+realIndex+',\\'REVIEW\\')">Повторити</button>',
      '<button type="button" onclick="setReviewStatus('+realIndex+',\\'LEARNED\\')">Вивчено</button>',
      '<button type="button" onclick="setReviewStatus('+realIndex+',\\'NEW\\')">Новий</button>',
      '<button type="button" class="danger" onclick="deleteLesson('+realIndex+')">Видалити</button>',
      '</div>'
    ].join("");
  }).join("");
  dataLog("LESSONS_LIST_RENDERED: "+filtered.length+" / "+REVIEW_FILTER);
}
function openLesson(index){const lessons=getLessons();const lesson=lessons[index];if(!lesson)return;document.getElementById("preview").srcdoc=lesson.html;ODIN_STATE.lessonCreated=true;ODIN_STATE.lastHtml=lesson.html;ODIN_STATE.lastModel=lesson.model;dataLog("LESSON_OPENED: "+lesson.title)}
function exportSavedLesson(index){const lessons=getLessons();const lesson=lessons[index];if(!lesson)return;renderDownload(lesson.html);dataLog("SAVED_LESSON_EXPORT_READY: "+lesson.title)}
function deleteLesson(index){const lessons=getLessons();const removed=lessons.splice(index,1)[0];setLessons(lessons);dataLog("LESSON_DELETED: "+(removed?removed.title:index));renderLessonsList()}
function clearSavedLessons(){localStorage.removeItem(ODIN_STORAGE_KEY);dataLog("ALL_LESSONS_CLEARED");renderLessonsList()}
function setFilter(filter){REVIEW_FILTER=filter;dataLog("FILTER_SET: "+filter);renderLessonsList()}
...
function executeOdinAction(){
  clearAll(); log("RUNNING"); runModeGuard("v3.14 REVIEW SYSTEM");
  ...
  renderDownload(html); log("EXPORT_DONE"); log("REVIEW_LAYER_READY"); log("DONE");
}
...
document.addEventListener("DOMContentLoaded",function(){
  ...
  document.getElementById("refreshLessonsBtn").addEventListener("click",renderLessonsList);

const ODIN_STORAGE_KEY = "odin_lessons_v3_14_1";
...
function dataLog(message) {
  const box = document.getElementById("dataLog");
  if (box.textContent === "DATA_WAITING") box.textContent = "";
  box.textContent += message + "\n";
}
...
function buildLesson(model) {
  ...
  return [
    ...
    '<h1>' + esc(model.title) + '</h1><span class="badge">ODIN v3.14.1 REVIEW READY</span>',
    ...
    '<section id="qa-marker"><h2>QA marker</h2><p>Урок пройшов ODIN v3.14.1 REVIEW PIPELINE.</p></section>',
...
function hardQaCheck(model, html) {
  ...
  const warning = text => messages.push({ level: "WARNING", text });
  ...
  model.examples.forEach((e, i) => {
    const n = i + 1;
    if (!e[0] || e[0].length < 4) error("Приклад " + n + ": DE відсутнє.");
    if (!e[1] || e[1].length < 4) error("Приклад " + n + ": ДП відсутній.");
    if (!e[2] || e[2].length < 4) error("Приклад " + n + ": СД відсутній.");
    if (e[1] === e[2]) warning("Приклад " + n + ": ДП і СД однакові.");
  });

  model.vocab.forEach((w, i) => {
    const n = i + 1;
    if (!w[0] || w[0].length < 2) error("Словник " + n + ": DE відсутнє.");
    if (!w[1] || w[1].length < 2) error("Словник " + n + ": UA відсутній.");
  });

  const deDups = findDuplicates(model.examples, item => item[0] || "");
  if (deDups.length > 0) error("Є дублікати прикладів DE: " + deDups.join(", "));
...
function renderQaReport(report) {
  const rows = report.messages.map(m =>
    '<div class="qa-item ' + (m.level === "ERROR" ? "qa-error" : m.level === "WARNING" ? "qa-warning" : "qa-info") + '"><b>' + m.level + ':</b> ' + esc(m.text) + '</div>'
  ).join("");
...
function renderDownload(html) {
  ...
  document.getElementById("download").innerHTML = '<a class="download" href="' + url + '" download="odin_lesson_v3_14_1.html">Завантажити урок</a>';
}
...
function applyMode() {
  ...
  if (!doc) return;
  doc.querySelectorAll(".dp").forEach(el => el.style.display = ODIN_STATE.showDP ? "block" : "none");
  doc.querySelectorAll(".sd").forEach(el => el.style.display = ODIN_STATE.showSD ? "block" : "none");
}
...
function getLessons() {
  try { return JSON.parse(localStorage.getItem(ODIN_STORAGE_KEY) || "[]"); }
  catch (e) { return []; }
}

function setLessons(lessons) {
  localStorage.setItem(ODIN_STORAGE_KEY, JSON.stringify(lessons));
}

function saveCurrentLesson() {
  if (!ODIN_STATE.lessonCreated || !ODIN_STATE.lastHtml || !ODIN_STATE.lastQa || !ODIN_STATE.lastQa.passed) {
    dataLog("SAVE_BLOCKED: немає успішного уроку після QA.");
    return;
  }
  ...
  const record = {
    id: "lesson_" + Date.now(),
    title: ODIN_STATE.lastModel.title,
    createdAt: new Date().toLocaleString(),
    html: ODIN_STATE.lastHtml,
    model: ODIN_STATE.lastModel,
    qaStatus: ODIN_STATE.lastQa.hasWarnings ? "QA_PASSED_WITH_WARNINGS" : "QA_PASSED",
    reviewStatus: "NEW",
    reviewCount: 0,
    lastReviewed: null
  };

  lessons.unshift(record);
  setLessons(lessons);
  dataLog("LESSON_SAVED: " + record.title);
  renderLessonsList();
}

function statusClass(status) {
  return status === "LEARNED" ? "status-learned" : status === "REVIEW" ? "status-review" : "status-new";
}

function setReviewStatus(index, status) {
  const lessons = getLessons();
  if (!lessons[index]) return;
  lessons[index].reviewStatus = status;
  lessons[index].lastReviewed = new Date().toLocaleString();
  lessons[index].reviewCount = (lessons[index].reviewCount || 0) + 1;
  setLessons(lessons);
  dataLog("REVIEW_STATUS_SET: " + lessons[index].title + " → " + status);
  renderLessonsList();
}
...
function renderLessonsList() {
  ...
  if (filtered.length === 0) {
    box.innerHTML = '<p class="muted">Немає уроків для цього фільтра.</p>';
    dataLog("LESSONS_LIST_EMPTY: " + REVIEW_FILTER);
    return;
  }
...
function openLesson(index) {
  ...
  if (!lesson) return;
  document.getElementById("preview").srcdoc = lesson.html;
  ODIN_STATE.lessonCreated = true;
  ODIN_STATE.lastHtml = lesson.html;
  ODIN_STATE.lastModel = lesson.model;
  dataLog("LESSON_OPENED: " + lesson.title);
}

function exportSavedLesson(index) {
  const lesson = getLessons()[index];
  if (!lesson) return;
  renderDownload(lesson.html);
  dataLog("SAVED_LESSON_EXPORT_READY: " + lesson.title);
}

function deleteLesson(index) {
  const lessons = getLessons();
  const removed = lessons.splice(index, 1)[0];
  setLessons(lessons);
  dataLog("LESSON_DELETED: " + (removed ? removed.title : index));
  renderLessonsList();
}

function clearSavedLessons() {
  localStorage.removeItem(ODIN_STORAGE_KEY);
  dataLog("ALL_LESSONS_CLEARED");
  renderLessonsList();
}

function setFilter(filter) {
  REVIEW_FILTER = filter;
  dataLog("FILTER_SET: " + filter);
  renderLessonsList();
}
...
function executeOdinAction() {
  clearAll();
  log("RUNNING");
  runModeGuard("v3.14.1 REVIEW SYSTEM");
  ...
  if (!report.passed) {
    document.getElementById("download").innerHTML = '<div class="qa-item qa-error">EXPORT BLOCKED: HARD QA FAILED</div>';
    log("EXPORT_BLOCKED");
    return;
  }
